...
export interface RunPromptInvocationInput{workspaceId:string;promptId:string;promptVersionId?:string;variableValues:Record<string,unknown>;modelOverride?:{provider?:string;modelName?:string;temperature?:number;maxTokens?:number;jsonMode?:boolean;streaming?:boolean}}
...
export class PromptInvocationService{
 ...
 async run(input:RunPromptInvocationInput,onProgress?:(progress:InvocationProgress)=>void):Promise<RunPromptInvocationResult>{
  ...
  const provider=input.modelOverride?.provider??snapshot.modelProvider,modelName=input.modelOverride?.modelName??snapshot.modelName,streaming=input.modelOverride?.streaming??true,temperature=input.modelOverride?.temperature??snapshot.temperature,maxTokens=input.modelOverride?.maxTokens??snapshot.maxTokens,jsonMode=input.modelOverride?.jsonMode??snapshot.outputType==='json'
  ...
  try{const key=await this.modelKey(provider,modelName),messages:Message[]=[...(rendered.renderedSystemPrompt?[{role:'system' as const,content:rendered.renderedSystemPrompt}]:[]),{role:'user',content:rendered.renderedUserPrompt}]
   let response:LLMResponse|undefined
   if(streaming){let output='';await this.llm.sendMessageStream(messages,key,{onToken:token=>{firstToken??=performance.now();output+=token;invocation.rawOutput=output;onProgress?.({status:'running',output})},onComplete:value=>{response=value},onError:()=>{}});invocation.rawOutput=response?.content??output}else{response=await this.llm.sendMessageStructured(messages,key);invocation.rawOutput=response.content}
